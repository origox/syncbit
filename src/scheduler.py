    ...
    def sync_data(self) -> None:
        ...
        while retry_count <= max_retries:
            try:
                ...
                logger.warning(f"Rate limited during sync (retry {retry_count}/{max_retries}), "
    ...
    def backfill_data(self) -> None:
        ...
        try:
            ...
            if last_synced:
                ...
                logger.info(f"Starting backfill from configured date {start_date.strftime('%Y-%m-%d')}")
            else:
                ...
                logger.info(f"Starting backfill from first available date {start_date.strftime('%Y-%m-%d')}")
            ...
            logger.info(f"Backfilling data from {start_date.strftime('%Y-%m-%d')} "
            ...
            try:
                ...
                if len(batch) >= batch_size or current_date == end_date:
                    ...
                    if batch:
                        ...
                        logger.info(f"Synced batch: {successful} days written, "
                ...
                logger.warning(f"Rate limited on {current_date.strftime('%Y-%m-%d')} "
                             f"({consecutive_rate_limits}/{max_consecutive_rate_limits})")
                
                # Write partial batch before handling error
                if batch:
                    logger.info(f"Writing partial batch of {len(batch)} days before rate limit wait...")
                    successful, failed = self.writer.write_multiple_days(batch)
                    total_successful += successful
                    total_failed += failed
                ...
                if consecutive_rate_limits >= max_consecutive_rate_limits:
                    logger.error(f"Stopping backfill after {consecutive_rate_limits} consecutive rate limits. "
